...
Namespace Ecospace
    ...
    Public Class cGridEcospaceResultsGear
        ...
        Protected Overrides Sub InitStyle()
            ...
            ' Catch (Start)
            Me(0, 2) = New cEwEColumnHeaderCell(SharedResources.HEADER_CATCH_START)
            ' Catch (End)
            Me(0, 3) = New cEwEColumnHeaderCell(SharedResources.HEADER_CATCH_END)
            ' Catch (E/S)
            Me(0, 4) = New cEwEColumnHeaderCell(SharedResources.HEADER_CATCH_E_OVER_S)
            ' Value (Start)
            Me(0, 5) = New cEwEColumnHeaderCell(SharedResources.HEADER_VALUESTART)
            ' Value (End)
            Me(0, 6) = New cEwEColumnHeaderCell(SharedResources.HEADER_VALUEEND)
            ' Value (E/S)
            Me(0, 7) = New cEwEColumnHeaderCell(SharedResources.HEADER_VALUEES)
            ' Cost (Start)
            Me(0, 8) = New cEwEColumnHeaderCell(SharedResources.HEADER_COST_START)
            ' Cost (End)
            Me(0, 9) = New cEwEColumnHeaderCell(SharedResources.HEADER_COST_END)
            ' Cost (E/S)
            Me(0, 10) = New cEwEColumnHeaderCell(SharedResources.HEADER_COST_E_OVER_S)
            ' Effort (E/S)
            Me(0, 11) = New cEwEColumnHeaderCell(SharedResources.HEADER_EFFORTES)
